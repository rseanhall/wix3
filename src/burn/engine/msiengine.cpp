    ...
    )
{
    ...
    for (DWORD i = 0; i < cProperties; ++i)
    {
        ...
        // build part
        hr = pfnStrAllocFormatted(&sczProperty, L" %s%=\"%s\"", pProperty->sczId, sczEscapedValue);
        ExitOnFailure(hr, "Failed to format property string part.");

        // append to property string
        hr = pfnStrAllocConcat(psczProperties, sczProperty, 0);
    ...
    )
{
    ...
    PFN_STRALLOC pfnStrAlloc = fZeroOnRealloc ? StrAllocSecure : StrAlloc;
    ...
    hr = pfnStrAlloc(psczEscapedValue, cch + cchEscape + 1); // character count, plus escape character count, plus null terminator
